#!/usr/bin/env python3
"""
Import direct Parquet -> PostgreSQL (miroir de l'import DuckDB)

Objectifs (d'après le brief) :
- Importer des fichiers Parquet dans PostgreSQL
- Éviter les doublons via une table de log (import_log)
- Fournir des statistiques de contrôle
- Ne pas utiliser d'import "inutile" (pas de glob/argparse/pathlib) :
  -> découverte des fichiers via os.listdir

Dépendances côté Python :
  - pandas (lecture Parquet + to_sql)
  - sqlalchemy (exécuter du SQL + transactions)
  - psycopg2-binary (driver PostgreSQL, paramétré via database.py)
"""
